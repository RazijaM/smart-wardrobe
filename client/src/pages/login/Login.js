...
function Login({ setAuth }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 401) {
        setErrors((prev) => ({ ...prev, form: 'Pogrešan username/email ili lozinka.' }));
        setLoading(false);
        return;
      }
      ...
      Cookies.set('authData', token, { path: '/' });
      setAuth({ token, user });
      navigate('/');
    } catch (error) {
      setErrors((prev) => ({ ...prev, form: 'Pogrešan username/email ili lozinka.' }));
  ...
  return (
    <div className="flex flex-col justify-center items-center p-4 min-w-[400px] max-w-md">
      <h1 className="text-2xl font-bold text-dusty-900 mb-4">Prijava</h1>
      <form onSubmit={handleSubmit} id="login_form" className="w-full space-y-4">
        {errors.form && (
