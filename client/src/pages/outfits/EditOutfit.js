...
function EditOutfit() {
  ...
  useEffect(() => {
    Promise.all([api(`/outfits/${id}`), api('/clothes')])
      .then(([outfit, allClothes]) => {
        setForm({
          name: outfit.name || '',
          occasion: outfit.occasion || '',
          season: outfit.season || '',
          clothingIds: (outfit.items || []).map((i) => i.clothingId),
        });
  ...
  const toggleCloth = (clothId) => {
    setForm((prev) => ({
      ...prev,
      clothingIds: prev.clothingIds.includes(clothId)
        ? prev.clothingIds.filter((cid) => cid !== clothId)
        : [...prev.clothingIds, clothId],
    }));
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api(`/outfits/${id}`, { method: 'PUT', body: form });
      navigate('/outfits');
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-dusty-900 mb-4">Uredi outfit</h1>
        ...
        <div>
          ...
          <div className="border border-dusty-200 rounded-btn p-4 max-h-64 overflow-y-auto space-y-3">
            {Object.keys(byCategory).length === 0 ? (
              <p className="text-dusty-500 text-sm">Nema komada u garderobi.</p>
            ) : (
